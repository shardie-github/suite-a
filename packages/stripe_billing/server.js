...
app.get("/stripe/ok", (_req,res)=>res.json({ oktrue, stripe !!stripe }));
...
app.post("/stripe/checkout", async (req,res)=>{
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url (process.env.APP_URL || "http//localhost3000") + "/success",
    cancel_url (process.env.APP_URL || "http//localhost3000") + "/cancel"
  });
